/*
  Compile with gcc -O3 galaxy.c -o galaxy -lm
  Run sequentially with srun -n 1 galaxy input.txt input_random.txt outfile.txt
 */
...
int main(int argc, char *argv[])
{
  ...
  for ( int i = 0; i < Nooflines_Sim; ++i ) 
    {
      //if ( i > 0 && (i/10000)*10000 == i ) printf("     %6d\n",i);
      for ( int j = i+1; j < Nooflines_Sim; ++j ) 
	{
